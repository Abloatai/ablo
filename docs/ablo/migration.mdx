---
title: "Version History & Migration Guide"
description: "Every breaking change in @abloatai/ablo, what to change, and which version introduced it."
---

This page is the breaking-changes-first companion to the [Changelog](/changelog).
The changelog tells the story of each release; this page tells you exactly what
to change when you upgrade.
...
<Warning>
`ablo.tasks` is — and always was — the schema `tasks` **model** proxy. Only the
agent-work *resource* of the same name was removed. If you have a `tasks` model
in your schema, it is unaffected.
</Warning>
...
prompt produced" filter (it keyed off `caused_by_task_id`); new writes leave
that column `null`.

<Note>
The server-side `agent_tasks` table, the `caused_by_task_id` delta column, and
the `agent_actions_log` hash-chain are intentionally **kept but dormant** — they
are load-bearing for the tamper-evident audit chain. The dead `/v1/tasks` and
`/api/agent/turn` route handlers were removed.
</Note>
...
<Note>
This — not 0.9.x — is the release where `useAblo()` became the one React read
path. If you are coming from a 0.6.x or earlier app, this is your biggest hook
migration.
</Note>
...
For the full chronological history, see the [Changelog](/changelog).
