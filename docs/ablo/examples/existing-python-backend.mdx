...
// authEndpoint points at the route that mints a per-user ek_ — the browser
// holds no secret. Build the client once at module scope, not per render.
const ablo = Ablo({
  schema,
...
The selector reads through the synchronous `get` / `claim.state` reads (never
the async `retrieve`/`list`), so React uses the same model resource as writes
and agents.
