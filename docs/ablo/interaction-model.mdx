...
```
Schema -> Model read -> Claim -> Model update -> Confirmation
```
...
| `Model` | State | The generated `ablo.<model>` resource. Use `retrieve`, `get`, `create`, `update`, and `delete`. |
| `Claim` | State | Pre-write coordination. It says what this actor is preparing to change. |
...
- `user` — a human, authenticated via session. Uses the live (WebSocket) client.
- `agent` — an AI process acting on behalf of a human, authenticated via a capability minted from that human's session. Uses the stateless HTTP client — see [Agents](/agents).
...
Claims broadcast across the org. When `agent:task-writer` claims a task to
update it, schema clients can see the current holder through
`ablo.tasks.claim.state({ id })` and the waiting line through
`ablo.tasks.claim.queue({ id })`. Callers decide whether to yield, wait, or
fail fast.
...
Declare schema, read state, claim the row, update the model, and wait for confirmation.
