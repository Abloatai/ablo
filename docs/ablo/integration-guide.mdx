...
one write with ablo.<model>.update({ id, data, readAt, onStale: 'reject',
...
Use `get`, `list`, and `count` for synchronous local reads after data has
loaded.
...
await ablo.tasks.update({
  id: taskId,
  data: { status: 'done', summary: await summarize(task) },
  readAt: snap.stamp,
  onStale: 'reject',
  wait: 'confirmed',
});
