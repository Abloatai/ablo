...
schema, point it at your database, and create the tables. Today the path is
**bring your own Postgres** — like Prisma, you set `DATABASE_URL` and `ablo
migrate` provisions your tables. Your `defineSchema(...)` is the single source of
truth — the CLI lowers it to SQL through one engine (`generateProvisionPlan` /
`generateMigrationPlan` in `@abloatai/ablo/schema`).

```bash
npx ablo init                 # scaffold ablo/schema.ts + client, write .env
# set DATABASE_URL in .env
npx ablo migrate              # create your schema's tables in DATABASE_URL
```

The hosted commands below (`ablo login`, `ablo dev`, `ablo schema push`) target
**Ablo-managed Postgres**, which is a [roadmap](/roadmap) offering. Use `ablo
migrate` against your own `DATABASE_URL` for now.
...
Two front doors to the same engine. Use `migrate` today — your app owns the
database and it applies to `DATABASE_URL`. `schema push` (and `dev`) run the
hosted path, where the server applies to Ablo-managed Postgres and version-gates
connecting clients; that hosted storage is a [roadmap](/roadmap) offering, so
`migrate` is the current default.
