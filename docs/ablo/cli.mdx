...
Postgres connector, you set `DATABASE_URL` and `ablo migrate` provisions tables.
Your `defineSchema(...)` is the single source of truth — the CLI lowers it to SQL
through one engine (`generateProvisionPlan` / `generateMigrationPlan` in
...
- **Prompts hang** — `ablo init` and bare `ablo mode` prompt. Use
  `ablo init --yes` and `ablo mode test|live` (with the argument). `init` also
...
The hosted commands below (`ablo login`, `ablo dev`, `ablo push`) target
**Ablo-managed Postgres**, which is a [roadmap](/roadmap) offering. Use a
[Data Source](/data-sources) for existing app tables, or `ablo migrate` when you
...
| --- | --- | --- |
| `ablo init` | Scaffold `ablo/` (`schema.ts`, client, Data Source endpoint, optional agent / component), write `.env`, install the SDK. Offers to log in at the end. Interactive by default; non-interactive with `--yes`/no-TTY. | `--yes`, `--framework`, `--auth`, `--storage`, `--no-agent`, `--no-pull`, `--no-install`, `--no-login` |
...
| `ablo check` | **Adoption** — verify your *existing* tables fit the schema (read-only, no DDL). | `--schema <path>`, `--export <name>`, `--app-schema <name>` |
...
The table-adoption front door. Instead of migrating (DDL on your database), Ablo
...
Postgres connector and want Ablo to provision tables in `DATABASE_URL`. `push`
(and `dev`) run the hosted path, where the server applies to Ablo-managed
Postgres and version-gates connecting clients; that hosted storage is a
[roadmap](/roadmap) offering.
