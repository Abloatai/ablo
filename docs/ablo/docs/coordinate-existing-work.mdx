---
title: "Coordinate Existing Work"
description: "Add Ablo coordination without replacing the application that already works."
sidebar:
  icon: route
---
...
|---|---|---|
| The claimed target is an Ablo model row and the final write goes through that model. | Row-backed claim; pass the returned `claim` to the write. | [Coordination](/coordination) |
| The result depends on an Ablo row that may change while work runs. | `read(...)` the premise and pass it through `reads`. | [Concurrency Convention](/concurrency-convention) |
| Several Ablo writes must all land or none may land. | One `commits.create(...)`. | [API Reference](/api) |
| A person needs live state, presence, or reactive local reads. | WebSocket client for that human interface. Workers stay on HTTP. | [React](/react) |
| The operation sends email, charges money, writes a file, or calls another provider. | That system's idempotency key or an application outbox. | [Idempotency](/idempotency) |
...
For setup, read the [Integration Guide](/integration-guide). The complete
