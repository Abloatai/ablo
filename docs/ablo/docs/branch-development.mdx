---
title: "Branch-first development"
description: "Understand exactly what `ablo dev` prepares, what it watches, and how each Git branch gets an isolated Ablo schema and credential."
sidebar:
  icon: git-branch
---
...
[API Keys](/api-keys#start-here-the-normal-workflow) for the complete
...
through [Connect Your Database](/data-sources). Never point a child branch at
...
- [Quickstart](/quickstart) — install, connect, define a schema, and write.
- [CLI](/cli) — the complete command surface.
- [Schema Contract](/schema-contract) — what the registered schema controls.
- [Connect Your Database](/data-sources) — Postgres roles, WAL, and database ownership.
- [Deployment](/deployment) — production schema rollout.
