...
description: "Who is connecting, and which slice of state they are allowed to see."
sidebar:
  icon: fingerprint
---

Start with [Groups and shared context](/groups) for how data membership,
...
[account multiplayer walkthrough](/examples/account-multiplayer).
...
See [the group lifecycle](/groups#a-participants-lifecycle) for loading,
...
[Groups and shared context](/groups#one-context-several-decisions) for how
...
runs the [Coordination](/coordination) `claim` loop is, to the scoping layer,
...
3. **Presence follows those same groups.** A reactive client announces one
   ...
   exclusion. See [React](/react#usepeers-read-only-presence).
...
> [`list()`](/api#model-methods) is a separate axis named **`state`**
...
- [Integration Guide](/integration-guide) — `identityRoles`, backing modes, and the full app path.
- [React](/react) — the complete `<AbloProvider>` prop surface.
- [API Keys](/api-keys) — server-side keys for the public API.
