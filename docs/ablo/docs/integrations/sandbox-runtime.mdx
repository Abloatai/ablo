...
description: "Run an Ablo agent with OS-enforced filesystem and network boundaries while its coordinated work remains durable outside the sandbox."
sidebar:
  icon: shield
---
...
- If the sandbox only returns a prepared result and the host process commits
  ...
  [Coordinate existing work](/coordinate-existing-work).
