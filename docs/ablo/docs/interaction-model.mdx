...
description: "The one write path every actor shares: load, claim, update, confirm."
sidebar:
...
await ablo.weatherReports.update({ id: claim.data.id, data: { status: 'ready' } });
```
...
await ablo.weatherReports.update({ id: claim.data.id, data: patch });
```

`get({ id })` is an async server read (await it). `claim.state({ id })` is a
...
> `claim.queue`, errors — is [Coordination](/coordination).
...
`readAt`, Ablo applies the `onStale` disposition:

- `reject` fails the commit, and the caller retries from scratch.
- `notify` holds the write, leaving the row as it stands, and returns a
  `StaleNotification` with the current value for the actor to reconcile.
- `overwrite` applies the write unconditionally, with no check.

The choice is per-commit, and there is no CRDT default: the policy is explicit.
The full taxonomy lives in [Concurrency Convention](/concurrency-convention).
