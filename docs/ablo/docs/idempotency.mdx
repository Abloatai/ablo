---
title: "Idempotency"
description: "Make a retried write safe: the same key never applies the same change twice."
sidebar:
  icon: repeat
---
...
- [Client Behavior](/client-behavior) — every write option, and which errors retry.
- [Guarantees](/guarantees) — what `queued` and `confirmed` promise.
- [Errors](/errors) — the full code registry, including every code named above.
