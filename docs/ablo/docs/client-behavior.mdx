...
description: "Guarded writes, claim behavior, and which errors are safe to retry."
sidebar:
  icon: monitor
---
...
coordination; point reads and administration remain HTTP. See [Transports](/transports)
for the lifecycle and [Options](/options) for the constructor. A human-facing
local graph is added through the [React client](/react).

Your database connects out of band — through logical replication (`npx ablo
connect`), or the signed [Data Source](/data-sources) endpoint as the
fallback for databases that can't grant replication — so the client holds only
`apiKey`, never a connection string. See
[Connect Your Database](/data-sources) for the full setup.
...
client, as shown in [React](/react). Do not reuse an old request's result as
