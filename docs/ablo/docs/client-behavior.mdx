...
{/* AUTO-GENERATED from packages/sync-engine/docs/client-behavior.md -- edit there, then run `npm run build:docs`. */}
...
connect`) or a signed [Data Source](/data-sources) endpoint — so the client
