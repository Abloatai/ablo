...
description: "One application connection, customer subject rules, and sessions minted from verified membership."
sidebar:
  icon: building-2
---
...
[supported connection setup](/data-sources). Updating TypeScript alone does
...
The [account multiplayer walkthrough](/examples/account-multiplayer) contains
...
[Groups and shared context](/groups) describes cache clearing and membership
...
See [Sessions](/sessions) for schema binding and [API keys](/api-keys) for
