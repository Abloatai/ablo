...
{/* AUTO-GENERATED from packages/sync-engine/docs/schema-contract.md -- edit there, then run `npm run build:docs`. */}
...
all covered in [Connect Your Database](/data-sources) (the single source of
...
For the shortest runnable path, start with [Quickstart](/quickstart). For a
production app, continue with [Integration Guide](/integration-guide).
