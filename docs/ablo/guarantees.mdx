...
Today Ablo backs your models with your own Postgres. You provide `DATABASE_URL`
and run `ablo migrate`, the same way Prisma owns your schema and connection. Ablo
records the coordination result for receipts, realtime fanout, and audit.

If you would rather not give Ablo a direct connection, keep your database behind
a signed Data Source endpoint instead. Ablo-managed (zero-database) storage is on
the [roadmap](/roadmap). See [Connect Your Database](/data-sources).
