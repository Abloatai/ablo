...
Existing app databases connect through a signed [Data Source](/data-sources):
your server writes the database, and Ablo coordinates the realtime stream. The
direct Postgres connector is optional for teams that explicitly want Ablo to open
a database connection and provision tables.
...
- **Ablo-managed storage** — a zero-database hosted option where Ablo stores the
  rows for your declared models, the way Stripe stores `Customer` objects.
