...
import { emitSchemaSource, type IRField, type IRRelation, type IRSchema } from './schemaIr';
...
export async function lowerDrizzleModule(mod: Record<string, unknown>): Promise<IRSchema> {
  ...
  for (const table of tables) {
    ...
    const reflectedColumns: ReflectedColumn[] = Object.entries(columns).map(([key, col]) => {
      const { kind, enumValues, note } = mapColumn(col);
      return { field: key, column: col.name, kind, enumValues, optional: !col.notNull, note, primary: col.primary };
    });
    ...
    reflected.push({ table: tableName, label: tableName, columns: reflectedColumns, relations });
  }
