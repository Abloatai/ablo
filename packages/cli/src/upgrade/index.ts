...
export async function upgrade(argv: readonly string[]): Promise<void> {
  ...
  for (const sf of files) {
    ...
    for (const jsx of sf.getDescendantsOfKind(SyntaxKind.JsxOpeningElement)) {
      ...
      if (stale) flag(jsx, '<AbloProvider> props', 'AbloProvider takes only `client` (+ fallback/onError). Build `const ablo = Ablo({ schema, session: { endpoint } })` and pass `client={ablo}`.');
    }
  }
