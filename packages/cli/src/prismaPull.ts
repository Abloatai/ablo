...
import { camelToSnake, emitSchemaSource, type IRField, type IRRelation, type IRSchema } from './schemaIr';
import { adoptReflection, type ReflectedColumn, type ReflectedTable } from './schemaSource';
...
export function parsePrismaSchema(src: string): IRSchema {
  ...
  for (const b of modelBlocks) {
    ...
    for (const line of b.body.split('\n')) {
      ...
      columns.push({
        field: raw.name,
        // Prisma states a column only when it overrides the default, so the
        // reflection restores what the engine would have derived.
        column: column ?? camelToSnake(raw.name),
        kind,
        enumValues,
        optional: raw.optional,
        note,
        primary: /@id\b/.test(raw.attrs),
      });
    }

    // A Prisma model is named separately from the table it maps to, so a
    // decline is reported under the name the customer would search for.
    reflected.push({ table: tableOf.get(b.name) ?? b.name, label: b.name, columns, relations });
  }

  return adoptReflection(reflected);
}
