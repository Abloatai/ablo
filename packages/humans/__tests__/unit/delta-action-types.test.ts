...
describe('Delta action type constants', () => {
  ...
  it('the canonical runtime schema accepts exactly those 8 action letters', () => {
    expect(syncDeltaActionSchema.options).toEqual(['I', 'U', 'D', 'A', 'V', 'C', 'G', 'S']);
    for (const action of ['I', 'U', 'D', 'A', 'V', 'C', 'G', 'S']) {
      expect(syncDeltaActionSchema.safeParse(action).success).toBe(true);
    }
  });

  it.each(['M', 'X', 'INSERT'])('rejects non-canonical action code %s', (action) => {
    expect(syncDeltaActionSchema.safeParse(action).success).toBe(false);
  });
});
...
describe('Incremental sync flow fixtures', () => {
  ...
  it('constructs a GroupAdded + N Covering sequence with sequential sync IDs', () => {
    ...
    if (!first) throw new Error('expected the group-added delta at index 0');
