/**
 * Property-based tests for MutationQueue.
 *
      ...
      fc.asyncProperty(
        ...
        async (opSequence) => {
          ...
          await new Promise<void>((resolve) => {
            const check = () => {
              if (resolvedCount >= createdCount) {
                queue.off('transaction:completed', check);
                queue.off('transaction:failed', check);
                resolve();
              }
            };
            // Registered AFTER the counting listeners above, so each event
            // increments before check() re-evaluates.
            queue.on('transaction:completed', check);
            queue.on('transaction:failed', check);
            check();
          });
