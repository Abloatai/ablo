...
type _WsRetrieveIsBareRow = Expect<
  Equal<
    Awaited<ReturnType<ModelOperations<TaskRow, TaskRow>['retrieve']>>,
    TaskRow | undefined
  >
>;
...
type _WsListIsBareArray = Expect<
  Equal<Awaited<ReturnType<ModelOperations<TaskRow, TaskRow>['list']>>, TaskRow[]>
>;

// ── Canned rows served at the materializer hydration boundary ───────────────

const CANNED_ROWS: readonly TaskRow[] = [
  { id: 't1', title: 'hello' },
  { id: 't2', title: 'world' },
];
...
function buildWsProxy(): ModelOperations<TaskRow, TaskRow> {
  ...
  const toModel = (row: TaskRow): Model =>
    Object.assign(new TaskModel({ id: row.id }), { title: row.title });
  ...
  const hydrationStub = {
    fetch: <T>(
      ...
      let rows: readonly TaskRow[] = CANNED_ROWS;
  ...
  return createModelProxy<TaskRow, TaskRow>(
    'tasks',
    'Task',
    pool,
...
describe('CONTRACT: retrieve/list transport parity', () => {
  describe('WebSocket proxy surface — bare row, no envelope', () => {
    it('retrieve({ id }) resolves the bare row', async () => {
      const tasks = buildWsProxy();
      const hit = await tasks.retrieve({ id: 't1' });
    ...
    it('retrieve({ id }) resolves undefined on a miss (bare absence, no envelope)', async () => {
      const tasks = buildWsProxy();
      await expect(tasks.retrieve({ id: 'nope' })).resolves.toBeUndefined();
    });
