 ...
 *
 * CONTRACT — the human materializer resolves typed read()/list() values.
 *
...
// WS surface: read resolves the BARE row (or undefined) — no envelope.
// eslint-disable-next-line @typescript-eslint/no-unused-vars
type _WsReadIsBareRow = Expect<
  Equal<
    Awaited<ReturnType<ModelOperations<ItemRow, ItemRow>['read']>>,
    ItemRow | undefined
  >
>;
// eslint-disable-next-line @typescript-eslint/no-unused-vars
type _WsGetIsBareRow = Expect<
  Equal<Awaited<ReturnType<ModelOperations<ItemRow, ItemRow>['get']>>, ItemRow | undefined>
>;
...
// `read`/`list` on the stateful client resolve through
...
function buildWsProxy(): ModelOperations<ItemRow, ItemRow> {
  ...
  const hydrationStub = {
    fetch: <T>(
      ...
      // `read` narrows via the tuple form `[['id', <id>]]`; honor it so
  ...
  return createModelOperations<ItemRow, ItemRow>(
    'items',
    'Item',
    pool,
    // read/list never touch the SyncClient (same stub idiom as
...
describe('CONTRACT: read/list transport parity', () => {
  describe('WebSocket proxy surface — bare row, no envelope', () => {
    it('get({ id }) resolves an observational bare row', async () => {
      const items = buildWsProxy();
      await expect(items.get({ id: 't1' })).resolves.toMatchObject({ id: 't1', title: 'hello' });
    });

    it('read({ id }) resolves the bare row', async () => {
      const items = buildWsProxy();
      const hit = await items.read({ id: 't1' });
    ...
    it('read({ id }) resolves undefined on a miss (bare absence, no envelope)', async () => {
      const items = buildWsProxy();
      await expect(items.read({ id: 'nope' })).resolves.toBeUndefined();
    });
