...
describe('project pin', () => {
  it('accepts the project resolved from the key', () => {
    expect(() => assertExpectedProject('proj_mail', 'proj_mail')).not.toThrow();
  });

  it('fails closed before startup when the key belongs to another project', () => {
    expect(() => assertExpectedProject('proj_mail', 'proj_slides')).toThrow(
  ...
  it('keeps existing clients compatible when no pin is configured', () => {
    expect(() => assertExpectedProject(undefined, 'proj_any')).not.toThrow();
  });

  it('fails closed when a same-project key belongs to another branch', () => {
    expect(() => assertExpectedBranch('br_production', 'br_feature')).toThrow(
  ...
  it('accepts the exact branch and keeps an absent branch pin compatible', () => {
    expect(() => assertExpectedBranch('br_feature', 'br_feature')).not.toThrow();
    expect(() => assertExpectedBranch(undefined, 'br_feature')).not.toThrow();
  });
});
