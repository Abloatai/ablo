 ...
 *   5. Object-valued tracked changes stay structured-cloneable for the
 *      IndexedDB mutation journal.
 */
...
describe('Dynamic model observability (lazyObservable opt-in)', () => {
  ...
  it('preserves contentJSON through pool.createFromData (the real ingest path)', async () => {
    // `InstanceCache.createFromData` is what bootstrap + WebSocket deltas call.
  ...
  it('updateFromData does NOT record outbound changes (hydration must not echo)', () => {
    // A remote delta arriving via updateFromData must update the in-memory
    // value WITHOUT landing in `modifiedProperties` — otherwise applying a
    // server delta would queue a brand-new outbound mutation and the record
    // would echo forever.
    const registry = new ModelRegistry({ validateOnRegister: false, allowLateReferences: true });
    setActiveRegistry(registry);
  ...
  it('applyChanges DOES record outbound changes (local update must sync)', () => {
    // Regression guard: `proxy.update({ id, data })` routes through
  ...
  it('keeps object-valued outbound changes structured-cloneable', () => {
    const registry = new ModelRegistry({ validateOnRegister: false, allowLateReferences: true });
    setActiveRegistry(registry);
    const Cls = buildObservableModelClass('CloneableLayer', true);
    registerType(registry, 'CloneableLayer', Cls);

    const instance = new Cls({
      id: 'layer-cloneable',
      position: { x: 0, y: 0, width: 100, height: 100 },
    });
    const position = { x: 250, y: 300, width: 100, height: 100 };

    instance.applyChanges({ position });

    const changes = instance.getChanges();
    expect(changes).toEqual({ position });
    // IndexedDB uses structured clone. Deep-observing modifiedProperties used
    // to turn `position` into a MobX Proxy here, which throws DataCloneError
    // and makes the fail-closed journal discard the write before dispatch.
