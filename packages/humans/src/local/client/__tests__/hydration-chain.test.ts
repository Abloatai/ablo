/**
 * Test: Schema → ModelRegistry → InstanceCache.createFromData → addBatch
 *
 ...
 * 2. InstanceCache.createFromData resolves constructors from the registry
...
describe('Ablo model proxy naming', () => {
  it('defers wire-model observability until the cache exposes the row', () => {
    ...
    registerModelsFromSchema(schema, registry);
    ...
    const modelInstance = requireValue(hydratedModel, 'hydrated model');
    expect(isObservableProp(modelInstance, 'title')).toBe(false);

    pool.addBatch([modelInstance]);
    expect(isObservableProp(modelInstance, 'title')).toBe(false);

    expect(pool.get('cold-1')).toBe(modelInstance);
    expect(isObservableProp(modelInstance, 'title')).toBe(true);
    pool.stopGC();
  });
  ...
  it('uses schema keys for public proxies and typenames for pool lookups', async () => {
    ...
    // `logger`) that aren't on the public `AbloOptions` surface, so
    // we cast through `InternalAbloOptions`.
    const ablo = Ablo({
      schema,
...
function setupRegistryAndPool() {
  ...
  for (const [_schemaKey, modelDef] of Object.entries(testSchema.models)) {
    const modelName = requireValue(modelDef.typename, 'model typename');
  ...
  setActiveRegistry(registry);
  const pool = new InstanceCache({ maxSize: 10000 }, registry);
...
describe('Hydration chain: schema → registry → createFromData → pool', () => {
  ...
  it('createFromData produces a model when __typename matches', () => {
    ...
    const instance = requireValue(m, 'chat model');
    expect(instance.id).toBe('chat-1');
  ...
  it('getModelByName returns usable constructor', () => {
    ...
    const ModelConstructor = requireValue(Ctor, 'Chat model constructor');
    const instance = new ModelConstructor({ id: 'test', title: 'hi' });
    expect(instance.getModelName()).toBe('Chat');
  });
  ...
  it('createFromData works when dynamic class calls makeObservable()', () => {
    ...
    const instance = requireValue(m, 'test model');
    expect(instance.id).toBe('test-1');
    expect(instance.getModelName()).toBe('TestModel');
  });
...
describe('Schema computed getters', () => {
  ...
  it('metadataObject parses JSON string metadata', () => {
    ...
    const { pool } = setupWithComputed({
      metadataObject,
      icon: (self) => ((self.metadataObject as Record<string, unknown>)?.icon) ?? 'default-icon',
  ...
  it('metadataObject returns {} for null/invalid metadata', () => {
    ...
    const m1 = pool.createFromData({ __typename: 'TestChat', id: 'c3', title: 'A', metadata: null }) as unknown as Record<string, unknown>;
