...
import {
  ...
  persistenceIdentityMatches,
  type PersistenceIdentity,
} from './persistenceIdentity.js';

export type { PersistenceIdentity } from './persistenceIdentity.js';
...
export class DatabaseManager {
  ...
  async calculateDatabaseInfo(
    ...
    const dbName = await persistenceDatabaseName(identity, userVersion);

    // Check if we need to increment schema version
    const existingInfo = await this.getDatabaseInfo(dbName);
    if (existingInfo && !persistenceIdentityMatches(existingInfo, identity)) {
      throw new AbloConnectionError(
        'Local database identity mismatch. Refusing to open persisted data ' +
          'owned by a different authenticated plane.',
    ...
    const allUserDatabases = await this.getDatabasesForUser(identity.participantId);
  ...
  async registerDatabase(info: DatabaseInfo): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      const tx = metaDb.transaction(['databases'], 'readwrite');
  ...
  async getDatabaseInfo(name: string): Promise<DatabaseInfo | null> {
    ...
    return new Promise((resolve, reject) => {
      const tx = metaDb.transaction(['databases'], 'readonly');
      const store = tx.objectStore('databases');
      const request = store.get(name);

      request.onsuccess = () => { resolve(request.result ?? null); };
      request.onerror = () => { reject(indexedDbError(request.error, 'registry read')); };
    });
  }

  /** Remove only the named ownership records; the shared registry stays live. */
  async unregisterDatabases(names: readonly string[]): Promise<void> {
    const metaDb = this.metaDb;
    if (!metaDb || names.length === 0) return;
    return new Promise((resolve, reject) => {
      const tx = metaDb.transaction(['databases'], 'readwrite');
      const store = tx.objectStore('databases');
      for (const name of new Set(names)) store.delete(name);
      tx.oncomplete = () => { resolve(); };
      tx.onerror = () => { reject(indexedDbError(tx.error, 'registry cleanup')); };
      tx.onabort = () => { reject(indexedDbError(tx.error, 'registry cleanup')); };
    });
  }
  ...
  async getDatabasesForUser(userId: string): Promise<DatabaseInfo[]> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => { reject(indexedDbError(request.error, 'user database lookup')); };
    });
  }
  ...
  async getWorkspaceMetadata(db: IDBDatabase): Promise<WorkspaceMetadata | null> {
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => { reject(indexedDbError(request.error, 'workspace metadata read')); };
    });
  }
  ...
  async isModelPersisted(db: IDBDatabase, modelName: string): Promise<boolean> {
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => { reject(indexedDbError(request.error, 'model persistence read')); };
    });
  }
  ...
  async getAllModelPersistenceStates(db: IDBDatabase): Promise<Record<string, boolean>> {
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => { reject(indexedDbError(request.error, 'model persistence scan')); };
    });
  }
   ...
   */
  async deleteWorkspaceDatabase(dbInfo: DatabaseInfo): Promise<boolean> {
    const deleted = await deleteIDBWithTimeout(dbInfo.name);
    if (!deleted) {
      this.runtime.observability.breadcrumb(
        `Database deletion failed or was blocked: ${dbInfo.name}`,
        'sync.database',
        'error',
      );
      return false;
    ...
    if (this.metaDb) {
      ...
      await new Promise<void>((resolve, reject) => {
        const tx = metaDb.transaction(['databases'], 'readwrite');
        tx.objectStore('databases').delete(dbInfo.name);
        tx.oncomplete = () => { resolve(); };
        tx.onerror = () => { reject(indexedDbError(tx.error, 'registry delete')); };
        tx.onabort = () => { reject(indexedDbError(tx.error, 'registry delete')); };
      });
    }
    return true;
  }
  ...
  async getDatabaseStatistics(): Promise<{
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => { reject(indexedDbError(request.error, 'registry statistics read')); };
    });
  }
