...
- Preserve typed claim conflict context across error serialization and HTTP/WebSocket reconstruction. Claim errors now expose display-safe counterparty kind, description, expiry, and target separately from the opaque participant id. WebSocket claims no longer gain an implicit timeout when `timeoutMs` is omitted, and cancellation/release waits for correlated server cleanup confirmation.
