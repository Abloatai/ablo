 ...
 *
 *   ```ts
 *   layouts: model({ reportId: z.string().nullish() }, {
 *     // The report-owns-layout link is nullable and the layout is always
 *     // created first; marking it `defer` lets the report commit ahead of the
 *     // layout instead of sharing its insert-order slot.
 *     report: relation.belongsTo('reports', 'reportId', { defer: true }),
 *   }),
 *   ```
 */
export interface BelongsToOptions {
   ...
   * `belongsTo('report', 'reportId', { parent: true })` — the report is the parent.
   */
   ...
   * entity. Leave it off provenance or template pointers (`sourceSectionId`,
...
> implements RelationDef<Type, Target, Field, Options>
{
   ...
   * and reader stay type-safe — no double-cast smuggle through `Record<...>`.
   */
  _orderBy?: string;
...
export const relation = {
   ...
   * section: relation.belongsTo('sections', 'sectionId', { index: true }),
   *
   ...
   *
   * ```ts
   * blocks: relation.hasMany('blocks', 'sectionId'),
   * // → section.blocks returns all Block[] where sectionId === section.id
   *
   * blocks: relation.hasMany('blocks', 'sectionId', { orderBy: 'zIndex' }),
   * // → section.blocks returns Block[] sorted by zIndex ascending
   * ```
   */
