 ...
 * The taught verbs map onto the seam directly: `read` returns settled state,
 * `create`/`update`/`delete` are operations inside a `commit()` payload, and
 * `claim` is `claim()`. Presence (`join`) is deliberately absent: it is live observation
...
import type { ClientCommitReceipt } from '../commit/contract.js';
...
export interface TransactionLayer {
  ...
  read(model: string, id: string): Promise<ModelData | null>;

  /** Read settled rows matching a {@link ListQuery}. */
  list(model: string, query?: ListQuery): Promise<ModelData[]>;

  /**
   * Submit a batch of operations with its idempotent `clientTxId` and compact
   * `reads` evidence, then receive durable acceptance.
   */
