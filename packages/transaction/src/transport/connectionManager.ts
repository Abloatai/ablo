...
export class ConnectionManager {
  // Connection state, mirrored to the consumer via `onStateChange`.
  ...
  send(event: ConnectionEvent): void {
    ...
    this.logger.info('[ConnectionManager] Transition', {
      from: prevState,
      to: nextState,
      event: event.type,
    });
    ...
    try {
      ...
      this.logger.debug('[ConnectionManager] onStateChange threw', {
  ...
  private onEnterState(state: ConnectionState, event: ConnectionEvent): void {
    switch (state) {
      ...
      case 'probing_network':
        ...
        // sets the socket's `isManualClose=true` and suppresses its own
        ...
        // emit, before the socket's onclose checks the flag, so the timing
  ...
  private async runRefreshCredential(): Promise<void> {
    ...
    try {
      ...
      this.logger.debug('[ConnectionManager] Credential re-mint threw (transient)', { error });
      this.send({ type: 'RECONNECT_FAILED' });
    }
  }
  ...
  private async runReconnect(): Promise<void> {
    ...
    try {
      ...
      this.logger.debug('[ConnectionManager] Reconnect threw', { error });
      this.send({ type: 'RECONNECT_FAILED' });
    }
  }
  ...
  private scheduleBackoff(): void {
    ...
    if (this.attempt >= this.backoff.MAX_ATTEMPTS) {
      ...
      this.logger.debug('[ConnectionManager] Max retries — hard reload');
      this.observability.breadcrumb(
    ...
    this.watchdogTimer = setInterval(() => {
      ...
      if (isStuck) {
        ...
        if (this.stuckCycles >= MAX_STUCK_CYCLES_BEFORE_RELOAD && browserOnline) {
          this.logger.debug('[ConnectionManager] Watchdog: sustained stuck — hard reload');
          this.observability.breadcrumb(
        ...
        this.logger.info('[ConnectionManager] Watchdog: stuck — retry', {
          state: this.state,
