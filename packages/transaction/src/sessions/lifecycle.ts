 ...
 */
export type CredentialRefreshResult = CredentialProviderResult;

/** A credential re-mint hook. `Promise<string | null>` resolvers remain
 *  assignable — the widened result type is a superset. */
export type CredentialRefresher = CredentialProvider;
...
function normalizeExpiresAtMs(expiresAt: string | undefined): number | null {
...
export class CredentialLifecycle {
  ...
  /** Seeds the first mint through the same expiry-aware path as later refreshes. */
  accept(result: CredentialRefreshResult): boolean {
    const token = credentialToken(result);
    if (!token) return false;
    this.credentialExpiresAtMs = normalizeExpiresAtMs(credentialExpiry(result));
    ...
    const run = (async (): Promise<CredentialRefreshOutcome> => {
      try {
        ...
        if (!this.accept(result)) {
          ...
          // structured session_expired). Terminal — this routes to sign-out.
  ...
  private routeRefreshOutcome(outcome: CredentialRefreshOutcome): CredentialRecoveryOutcome {
    ...
    if (outcome === 'session_error') {
      // The long-lived login is gone (the mint reported session_expired). Report it;
