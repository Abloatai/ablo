 ...
 * expired`) and rejected write collisions through two channels:
...
export function formatClaim(e: ClaimEvent): string {
  ...
  const by = actor
    ? e.phase === 'rejected' || e.phase === 'lost'
      ...
      : ` — by ${actor}`
    : '';
  // The cause when there is one, with human-readable detail when supplied;
  // otherwise what the holder said they were doing. A line never
...
/** A rejected stale write or foreign-claim collision as one readable line. */
