...
interface CommitTransaction {
  ...
  /** The premise for the whole batch, forwarded to the executor so the server can detect stale-context writes. */
  reads?: ReadDependency[] | null;
  /** Durable premises (`track`) to register for the batch's participant, forwarded to the executor. */
  track?: TrackDependency[] | null;
  status: 'pending' | 'executing' | 'awaiting_delta' | 'completed' | 'failed';
  createdAt: number;
  attempts: number;
  /**
   * Transient failures that were NOT the transport being down. Waiting out a
   * dropped connection is unbounded on purpose — that is what lets a commit
   * survive a reconnect — so connection errors are deliberately not counted
   * here, and only a server that keeps rejecting can exhaust the lane.
   */
...
export class MutationQueue extends EventEmitter {
   ...
   */
  private takeOfflineFlushBatch(pending: QueuedMutation[]): QueuedMutation[] {
    ...
    while (remaining.length > 0) {
      ...
      try {
        ...
        const result = this.parseMutationCommitResult(
          await this.dispatchCommitBounded(durableEnvelope.operations, {
            idempotencyKey,
          }),
        );
  ...
  async delete(
    ...
    if (actualModelName === 'Activity') {
      ...
      const mockTransaction: QueuedMutation = {
        id: this.generateId(),
        type: 'delete',
        modelName: actualModelName,
        modelId: model.id,
   ...
   * next batched commit.
   */
  async unarchive(model: Model, context: UserContext): Promise<QueuedMutation> {
    this.assertDurableReplayOpen();
    // Use getModelName() rather than constructor.name, which is unreliable once
    // class names are minified.
    const actualModelName = model.getModelName();
        ...
        try {
          ...
          if (batchOps.length > 0) {
            ...
            try {
              ...
              const result = this.parseMutationCommitResult(
                await this.dispatchCommitBounded(operations, {
                  idempotencyKey: commitIdempotencyKey,
                }),
              );
   ...
   */
  private applyOptimisticCreate(model: Model, transaction: QueuedMutation): void {
    applyOptimisticCreate(this.optimisticUpdates, this, model, transaction);
  }

  private applyOptimisticUpdate(model: Model, transaction: QueuedMutation): void {
    applyOptimisticUpdate(this.optimisticUpdates, this, model, transaction);
  }

  private applyOptimisticDelete(model: Model, transaction: QueuedMutation): void {
