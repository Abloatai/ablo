...
export class LazyReferenceCollection<T extends Model> {
  ...
  private _setupObservationTracking(): void {
    ...
    const disposeOnObserved = onBecomeObserved(this, 'items', () => {
      ...
      if (this.parent && typeof (this.parent as any)._registerObservedCollection === 'function') {
        (this.parent as any)._registerObservedCollection(this);
      }
    });
    ...
    const disposeOnUnobserved = onBecomeUnobserved(this, 'items', () => {
      ...
      if (this.parent && typeof (this.parent as any)._unregisterObservedCollection === 'function') {
        (this.parent as any)._unregisterObservedCollection(this);
      }
    });
  ...
  private async _loadFromLocal(): Promise<T[]> {
    try {
      ...
      for (const data of rawData) {
        ...
        let model = this.objectPool.get(data.id) as T | undefined;
  ...
  dispose(): void {
    ...
    if (this._isBeingObserved && this.parent) {
      if (typeof (this.parent as any)._unregisterObservedCollection === 'function') {
        (this.parent as any)._unregisterObservedCollection(this);
      }
    }
