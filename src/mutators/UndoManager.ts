...
async function applyOps<S extends Schema>(tx: Transaction<S>, ops: InverseOp[]): Promise<void> {
  ...
  // used by the replay machinery.
  type Mutators = Record<
    string,
    {
      create: (data: Record<string, unknown>) => Promise<unknown>;
      update: (patch: { id: string } & Record<string, unknown>) => Promise<unknown>;
  ...
  for (const op of ops) {
    ...
    switch (op.kind) {
      ...
      case 'delete':
        ...
        break;
      case 'createMany':
        await m.createMany(op.data);
        break;
      case 'updateMany':
        await m.updateMany(op.patches);
        break;
      case 'deleteMany':
        await m.deleteMany(op.ids);
        break;
    }
  }
}
