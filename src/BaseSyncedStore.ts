...
import type { Database, BootstrapResult } from './Database';
import type { BootstrapData } from './sync/BootstrapHelper';
...
import { ModelScope } from './ObjectPool';
import { LazyReferenceCollection } from './LazyReferenceCollection';
import { postBootstrapRegistry, type PostBootstrapContext } from './PostBootstrapRegistry';
import type { Schema } from './schema/schema';
import { createReaderActions, type ReaderActions } from './react/useReader';
...
> {
  ...
  /**
   * Run post-bootstrap hooks. Provides a `PostBootstrapContext` exposing
   * the raw infrastructure (pool, db, orgId) plus a typed `allModelsOfType`
   * convenience. Hooks call pure data-loader functions directly; the SDK
   * no longer knows about any app-specific method names.
   */
  protected async runPostBootstrapHooks(): Promise<void> {
    try {
      const ctx: PostBootstrapContext = {
        pool: this.objectPool,
        db: this.database,
        orgId: this.userContext?.organizationId,
        helper: this.database.helper,
        allModelsOfType: (modelClass: abstract new (...args: never[]) => Model): Model[] =>
          this.objectPool.getByType(modelClass),
      };
      const hookStart = Date.now();
      await postBootstrapRegistry.executeAll(ctx);
      console.warn(`[sync-engine] Post-bootstrap hooks completed in ${Date.now() - hookStart}ms`);
    } catch (err) {
      console.warn('[sync-engine] Post-bootstrap hooks failed:', err instanceof Error ? err.message : String(err), err);
      getContext().observability.breadcrumb('Post-bootstrap hooks failed', 'sync.bootstrap', 'warning', {
  ...
  protected async checkSyncGroupShrinkage(): Promise<void> {
    ...
    try {
      ...
      const currentGroups = new Set([
        'default',
        `org:${this.userContext.organizationId}`,
        `user:${this.userContext.userId}`,
        ...(this.userContext.teamIds ?? []).map((id: string) => `team:${id}`),
      ]);
  ...
  *initialize(
    ...
    try {
      ...
      try {
        ...
        console.warn('[sync-engine] IDB hydration failed:', hydrateError);
      ...
      if (hasLocalData) {
        console.warn(`[sync-engine] IDB hydrated ${this.objectPool.size} objects, running post-bootstrap hooks`);
        yield this.runPostBootstrapHooks();
        this.dataReady = true;
        this.initialized = true;
        console.warn('[sync-engine] dataReady=true, initialized=true (from IDB cache)');
        this.updateSyncStatus({ state: 'syncing', progress: 50 });
      } else {
        console.warn('[sync-engine] No local data in IDB — waiting for server bootstrap');
      }
      ...
      console.warn(`[sync-engine] Bootstrap requirements: ${requirements.type}, hasLocalData: ${hasLocalData}`);
      if (requirements.type !== 'local') {
        if (hasLocalData) {
          // Background bootstrap — don't block UI
          console.warn('[sync-engine] Firing background bootstrap (non-blocking)');
          this.performBackgroundBootstrap(requirements, context, signal);
        } else {
          // First load — must wait for server data
          console.warn('[sync-engine] Awaiting server bootstrap (blocking)');
          yield this.executeBootstrapWithTimeout(
            async () => {
              await this.database.bootstrapFromServer(requirements, {
  ...
  protected setupWebSocketSync(context: UserContext, lastSyncId: number): void {
    ...
    this.syncWebSocket = new SyncWebSocket<TCollaboration>({
      ...
      syncGroups: [
        ...
        `user:${context.userId}`,
        ...(context.teamIds || []).map((id: string) => `team:${id}`),
      ],
      lastSyncId,
  ...
  protected processDeltaWithBatching(delta: SyncDelta): void {
    ...
    // Non-delete — batch for efficiency
  ...
  protected cascadeCancelTransactionsForDeletedParent(parentModelName: string, parentId: string): void {
    ...
    for (const { childModel, foreignKey } of childModels) {
      ...
      const ctor = reg.getModelByName(childModel);
      const children = ctor
        ? this.objectPool.getByType(ctor).filter((e) => {
            const data = e.toJSON() as Record<string, unknown>;
            return data[foreignKey] === parentId;
          })
        : [];
  ...
  protected async flushPendingDeltas(): Promise<void> {
    ...
    const dbResults = await this.database.processDeltaBatch(
      regularDeltas.map((d) => ({
        syncId: d.id,
    ...
    // Acknowledge + advance sync cursor
    const lastDelta = deduplicatedDeltas[deduplicatedDeltas.length - 1];
    if (lastDelta && lastDelta.id > this.lastAckedId) {
      this.syncWebSocket?.acknowledge?.(lastDelta.id);
      this.lastAckedId = lastDelta.id;
      this.highestProcessedSyncId = Math.max(this.highestProcessedSyncId, lastDelta.id);
    }
  ...
  /** Batch save */
  async saveMany<T extends { id: string; createdAt?: Date; updatedAt?: Date }>(
    models: T[]
  ): Promise<void> {
    for (const model of models) await this.save(model);
  }

  /** Optimized batch save */
  async saveManyOptimized<T extends { id: string; createdAt?: Date; updatedAt?: Date }>(
    entities: T[],
    options?: { skipValidation?: boolean }
  ): Promise<void> {
    if (entities.length === 0) return;
    const models = entities as unknown as Model[];
    ...
    // SyncClient handles: batch pool add, IDB write, transaction queue
    for (const model of models) {
      const isCreate = !this.objectPool.get(model.id);
      if (isCreate) {
        model.updatedAt = new Date();
  ...
  /** Batch delete */
  async deleteMany<T extends { id: string }>(models: T[]): Promise<void> {
    for (const model of models) await this.delete(model);
  }

  /** Optimized batch delete */
  async deleteManyOptimized<T extends { id: string }>(entities: T[]): Promise<void> {
    const models = entities as unknown as Model[];
    for (const model of models) {
      this.pendingDeletes.add(model.id);
      this.syncClient.delete(model);
   ...
   *
   * ```ts
   * const chat = store.query.chats.findById(chatId);  // Chat | undefined
  ...
  get query(): QueryNamespace<TSchema> {
    ...
    if (!this._queryProxy) {
      ...
      const store = this as unknown as Parameters<typeof createReaderActions>[2];
  ...
  findById(_modelClass: ModelConstructor<Model>, id: string): Model | undefined {
  ...
  create<K extends keyof TSchema['models'] & string>(
  ...
  ): import('./schema/schema').InferModel<TSchema, K> | null {
    if (!this.schema) {
      throw new AbloValidationError(
    ...
    return this.objectPool.create(wireTypename, data) as
      | import('./schema/schema').InferModel<TSchema, K>
      | null;
  }
   ...
   * narrow types should use the schema-typed `useQuery(schema, key)` hook
  ...
  protected updateSyncStatus(updates: Partial<SyncStatus>): void {
    console.warn(`[sync-engine] syncStatus → ${updates.state ?? this.syncStatus.state}`, updates.error ? `error: ${updates.error}` : '');
    runInAction(() => {
