...
// Drift guard, both directions. Assignability one way only would let the schema
// grow a fifth `by` value that no authoring branch can reach: the runtime would
// accept it while `resolvePolicy`'s switch below never saw it. Pinning the
// discriminants in both directions makes either side's addition a compile error.
type _PolicyInputMatchesSchema = PolicyInput extends z.input<typeof policyInputSchema> ? true : never;
type _SchemaByMatchesPolicyInput =
  z.input<typeof policyInputSchema>['by'] extends PolicyInput['by'] ? true : never;
const _policyInputAssignable: [_PolicyInputMatchesSchema, _SchemaByMatchesPolicyInput] = [
  true,
  true,
];
