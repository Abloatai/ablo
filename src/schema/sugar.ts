 ...
 *     CREATE/UPDATE/DELETE over the `batch_ack` wire protocol.
...
> {
  ...
  orgScoped?: boolean;
  /**
   * Template for `/mesh` scope derivation (e.g. `'matter:{id}'`). When
   * set, the entity is a valid scope key for `mesh.join`. Omit for
   * nested children whose access cascades from a parent.
   */
 ...
 * mutations via `batch_ack` — equivalent to setting
 ...
 * deltas but cannot emit mutations — any `batch_ack` op for this model
