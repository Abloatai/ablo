...
import type { InferModel } from '../schema/schema';
import type { ResolveSchema } from '../types/global';
import { useSyncContext } from './context';
import type { QueryViewOptions } from '../core/QueryView';
...
/**
 * Declarative query hook for entity collections.
 *
 * **Typed overload (recommended):**
 * ```ts
...
/**
 * Typed query (explicit schema arg): pass the schema + model key for
 * full type inference. Preferred when the schema hasn't been wired into
 * `SyncProvider` / the global `AbloSync` augmentation.
 *
 * ```ts
...
/**
 * Untyped query: pass a typename string. Returns Record<string, unknown>[].
 * Use one of the typed overloads for type safety.
 */
...
export function useQuery<T = Record<string, unknown>>(
  ...
  // Cast to Record<string, unknown> for the view — the view doesn't need
  // the full generic type, it works with the runtime values.
  const viewOptions: QueryViewOptions<Record<string, unknown>> | undefined = options
    ? {
        ...
        filter: options.filter as unknown as ((entity: Record<string, unknown>) => boolean) | undefined,
  ...
  return useReactive(() => view.results.slice());
}

/**
 * Look up a single entity by ID. Typed when schema is provided.
 *
 * ```ts
 * // Typed
 ...
 * ```
 */
/** Typed single-entity lookup (explicit schema). */
...
/** Untyped single-entity lookup (legacy). */
