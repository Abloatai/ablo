...
const EMPTY_INTENTS: readonly ResourceIntent[] = Object.freeze([]);

type ModelResourceSelector<R extends SchemaRecord, T, C> =
...
function readModelResult<T, C>(
  ...
  resource: ModelOperations<T, C> | undefined,
  id: string | undefined,
  initial: T | undefined,
...
export function useAblo<
  ...
  C = unknown,
>(
  select: ModelResourceSelector<R, T, C>,
  id: string,
  options: UseAbloModelOptions<T> & { readonly initial: T },
): UseAbloHydratedModelResult<T>;
export function useAblo<T, C>(
...
export function useAblo<
  ...
  C = unknown,
>(
  resourceOrSelect?: ModelOperations<T, C> | ModelResourceSelector<R, T, C> | AbloSelector<R, T>,
  id?: string,
  options?: UseAbloModelOptions<T>,
  ...
  const resource: ModelOperations<T, C> | undefined =
    typeof resourceOrSelect === 'function' && id !== undefined
      ? engine
        ? (resourceOrSelect(engine as Ablo<R>) as ModelOperations<T, C>)
        : undefined
  ...
  const selected = useReactive<T | undefined>(
    () => {
      ...
      return snapshotValue(resourceOrSelect(engine as Ablo<R>) as T);
    },
  );
  ...
  if (resourceOrSelect) return modelResult;
  return engine as Ablo<R> | null;
}
