...
export interface AgentResourceMutationOptions
  extends Omit<ResourceMutationOptions, 'intent'> {
...
export interface AgentResourceClient<T = Record<string, unknown>> {
  ...
  create(
    ...
    options?: AgentResourceMutationOptions & { readonly id?: string | null },
  ): Promise<CommitReceipt>;
  update(
    id: string,
    data: Record<string, unknown>,
    options?: AgentResourceMutationOptions,
  ): Promise<CommitReceipt>;
...
interface IntentCreateResponse {
  readonly intent?: ResourceIntent;
}
...
export function createProtocolClient(options: AbloApiClientOptions): AbloApi {
  ...
  function createAgentRunContext(agentClient: AbloApi, task: Task): AgentRunContext {
    return {
      ...
      ablo: agentClient,
      resource<T = Record<string, unknown>>(name: string): AgentResourceClient<T> {
        return createAgentResourceClient<T>(agentClient, name);
      },
    };
  }

  function createAgentResourceClient<T>(
    agentClient: AbloApi,
    name: string,
  ): AgentResourceClient<T> {
    const base = agentClient.resource<T>(name);
    ...
    return {
      ...
      create(
        data: Record<string, unknown>,
        mutationOptions?: AgentResourceMutationOptions & { readonly id?: string | null },
      ): Promise<CommitReceipt> {
      ...
      update(
        id: string,
        data: Record<string, unknown>,
        mutationOptions?: AgentResourceMutationOptions,
      ): Promise<CommitReceipt> {
        return withAgentIntent(
          agentClient,
          name,
  ...
  async function withAgentIntent(
    ...
    resourceName: string,
    id: string,
    mutationOptions: AgentResourceMutationOptions | undefined,
  ...
  ): Promise<readonly ResourceIntent[]> {
  ...
  async function waitForNoIntents(
    ...
    for (;;) {
      ...
      if (pollInterval == null) {
        throw new AbloValidationError(
          'Cannot wait for intents over the schema-less HTTP client without `pollInterval`. ' +
  ...
  async function applyBusyPolicy(
    target: Partial<ResourceTarget>,
    options?: BusyOptions,
    defaultPolicy: BusyOptions['ifBusy'] = 'return',
  ): Promise<void> {
    ...
    const intents = await listIntents(target);
    if (intents.length === 0) return;
    if (policy === 'fail') throw busyError(target, intents, 'resource_busy');
  ...
  async function retrieveResource<T>(
    ...
    const query = await requestJson<QueryResponse>(
      `/v1/resources/${encodeURIComponent(resourceName)}/${encodeURIComponent(id)}`,
      {
        method: 'GET',
      },
    );
...
function withAgentBusyDefault<T extends BusyOptions | undefined>(
  options: T,
): BusyOptions & NonNullable<T> {
  return {
    ifBusy: 'fail',
    ...(options ?? {}),
  } as BusyOptions & NonNullable<T>;
}

function stripAgentRuntimeOptions(
  options: AgentResourceMutationOptions | undefined,
): Omit<ResourceMutationOptions, 'intent' | 'ifBusy' | 'busyTimeout' | 'busyPollInterval'> | undefined {
  if (!options) return undefined;
  const {
    intent: _intent,
    ifBusy: _ifBusy,
    busyTimeout: _busyTimeout,
    busyPollInterval: _busyPollInterval,
    ...rest
  } = options;
  return rest;
}
