 ...
 *
 * ```ts
 * import { createMesh, session } from '@ablo/sync-engine/mesh';
 *
 * const mesh = createMesh({ schema });
 *
 * const participant = await mesh.join(myAgent, {
 *   scope: [{ entity: schema.models.matters, ids: 'deal-1' }],
 *   onBehalfOf: session({
 *     id: betterAuth.session.id,
 *     userId: betterAuth.session.userId,
 *     organizationId: currentOrg.id,
 *   }),
 * });
 * ```
 *
 * Every `mesh.join(...)` that runs under a logged-in browser tab uses
 * `session(...)`. Agent-spawn-agent flows use `agent(...)` — but those
 * rarely appear in customer code because `participant.join(child)`
 * does the attenuation automatically.
 *
...
import type { AgentRef, SessionRef } from './api';
