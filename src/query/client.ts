...
import { z } from 'zod';
import type { QueryBatch, QueryBatchResult } from './types';
...
export async function postQuery(
  ...
  try {
    ...
    if (!response.ok) {
      // Loud, grep-friendly prefix with `console.error` so operators
      // can alert on `[postQuery.error]`. Never throw — fire-and-forget
      // callers would kill Next.js router on unhandled rejection. Each
      // slot becomes `[]` so consumers keep array-iterating safely.
      console.error(
